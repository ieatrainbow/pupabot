...
# Configure logging
logging.basicConfig(filename=f'{config.patch}/log/log.txt',
                    filemode='a',
...
async def exception():
    logging.basicConfig(filename=f'{config.patch}/log/last_error.txt',
                        filemode='W',
                        format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                        datefmt='%H:%M:%S',
                        level=logging.ERROR)
    await bot.send_document(config.wiz_id, document=open(f'{config.patch}/log/last_error.txt', 'rb'))
